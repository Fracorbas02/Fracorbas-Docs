...
L'indice du challenge — *« the jukebox straight into the floor with the trimmings still attached »* — pointe vers une désérialisation YAML non sécurisée. Le serveur utilise `yaml.load()` (PyYAML 6.0.2) au lieu de `yaml.safe_load()`, ce qui permet l'exécution arbitraire de code via les tags `!!python/object/apply`.
...
Le mot de passe est celui de **root**, pas de l'utilisateur `ubuntu`.
